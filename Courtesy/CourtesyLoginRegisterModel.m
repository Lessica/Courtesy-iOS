...
- (void)callbackDelegateWithSucceesAccount:(NSString *)email isLogin:(BOOL)login {
...
- (void)makeRequest:(BOOL)login {
    if (![ValidateUtils isValidateEmail:_email]) {
    ...
    JSONObjectBlock handler = ^(id json, JSONModelError *err) {
        ...
        @try {
            ...
            if (errorCode == 0) {
                ...
                [self callbackDelegateWithSucceesAccount:_email isLogin:YES];
                return;
...
- (void)sendRequestRegister {
    ...
    JSONObjectBlock handler = ^(id json, JSONModelError *err) {
        ...
        @try {
            ...
            if (errorCode == 0) {
                [[GlobalSettings sharedInstance] setHasLogin:YES];
                [self callbackDelegateWithSucceesAccount:_email isLogin:NO];
                return;
